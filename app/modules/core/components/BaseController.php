...
class BaseController extends Controller
{
    ...
    {
        if (!Yii::$app->user->isGuest && Yii::$app->user->idParam !== '__bId') {
            $lastPost = Post::getLast();
            if ($lastPost) {
                $user = Yii::$app->user->identity;
                if ((int) $user->getNotificationSettings()->select('value')->where(['setting_id' => NotificationSetting::TYPE_BROWSER])->scalar() === NotificationSetting::VALUE_SELECTED) {
                    if (($userLastPostId = (int) $user->getNotificationLastNews()->select('post_id')->scalar()) < $lastPost->id) {
                        var_dump($userLastPostId);
                        $count = Post::find()->where(['>', 'id', $userLastPostId])->count();
                        Yii::$app->getSession()->setFlash(
                            'user_success',
                            Yii::t('app', "There is $count new news. <button class='btn' id='acceptNotification'>Ok</button>")
                        );
                    }
                }
