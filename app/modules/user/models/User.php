...
use app\modules\notifications\models\NotificationLastNews;
...
class User extends AppActiveRecord implements IdentityInterface, VerifyEmailInterface
{
    ...
    {
        return $this->hasMany(NotificationSetting::className(), ['user_id' => 'id']);
    }

    /**
     * @return ActiveQuery
     */
    public function getNotificationSettingsIfNot()
    {
        $types = NotificationType::find()->all();
        foreach ($types as $type) {
            NotificationSetting::get($this->id, $type->id);
        }
        return $this->hasMany(NotificationSetting::className(), ['user_id' => 'id']);
    }
     ...
     * @return ActiveQuery
     */
    public function getNotificationLastNews()
    {
        return $this->hasOne(NotificationLastNews::className(), ['user_id' => 'id']);
    }

    public function setLastNewsViewed($lastPostId)
    {
        if (!$model = $this->notificationLastNews) {
            $model = new NotificationLastNews(['user_id' => $this->id]);
        }
        $model->post_id = $lastPostId;
        return $model->save();
    }
