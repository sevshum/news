...
class UsersController extends BaseController
{
    ...
    public function actionProfile($tab = null)
    {
        ...
        $settings = $user->getNotificationSettingsIfNot()->indexBy('id')->all();
        $saved = false;
        if (Model::loadMultiple($settings, Yii::$app->request->post()) && Model::validateMultiple($settings)) {
            foreach ($settings as $setting) {
                $setting->save(false);
            }
            Yii::$app->getSession()->setFlash('user_success', Yii::t('app', 'Notifications updated successfully.'));
            $saved = true;
        }
        if ($user->load($_POST)) {
        ...
        return $this->render('profile', ['user' => $user, 'tab' => $tab, 'settings' => $settings]);
    }
